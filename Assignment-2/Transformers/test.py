...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test Transformer Seq2Seq model for English-Hindi transliteration"
    )
    ...
    # Training hyperparameters (for model directory naming)
    parser.add_argument(
...
def test(args):
    ...
    model = TransformerModel(
        input_vocab_size=len(src_vocab),  # Source vocab size
        output_vocab_size=len(tgt_vocab),  # Target vocab size
        ...
        pad_token_id=test_dataset.tgt_char2idx["<PAD>"],  # <PAD>
        sos_token_id=test_dataset.tgt_char2idx["<SOS>"],  # <SOS>
        eos_token_id=test_dataset.tgt_char2idx["<EOS>"],  # <EOS>
        unk_token_id=test_dataset.tgt_char2idx["<UNK>"],  # <UNK>
    ).to(device)
    ...
    for batch in tqdm(test_loader, desc="Testing", total=len(test_loader)):
        ...
        with torch.no_grad():
            output_tokens_greedy = model.generate_greedy(src)
