...
def make_tag_given_tag(tagged_sentences_train, tags):
    tag_given_tag = {}
...
def make_word_given_tag(tagged_sentences_train, tags):
    ...
    with open("data/word_given_tag.json", "w") as f:
        json.dump(word_given_tag, f)
    with open("data/total_given_tag.json", "w") as f:
...
def main():
    ...
    with open("../data/train_data.pkl", "rb") as f:
        tagged_sentences_train = pkl.load(f)

    with open("../data/test_data.pkl", "rb") as f:
        tagged_sentences_test = pkl.load(f)
    ...
    make_tag_given_tag(tagged_sentences_train, tags)
    make_word_given_tag(tagged_sentences_train, tags)
    ...
    with open(f"data/tag_given_tag.json", "r") as f:
        tag_given_tag = json.load(f)

    with open(f"data/word_given_tag.json", "r") as f:
        word_given_tag = json.load(f)

    with open(f"data/total_given_tag.json", "r") as f:
        total_given_tag = json.load(f)

    with open(f"data/tag_to_index.json", "r") as f:
    ...
    confusion_matrix = confmat(predicted, actual, tag_to_index)
    ...
    print(
        metrics.classification_report(
            actual, predicted, labels=list(tag_to_index.keys()), digits=4
        )
    )
