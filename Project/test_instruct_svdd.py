...
def generate(
    ...
    hates_emb = sentence_model.encode(
        hates, convert_to_tensor=True, normalize_embeddings=True
    )
    ...
    for num_block in range(num_blocks):
        ...
        for i in tqdm(range(steps)):
            ...
            for c in range(candidates.shape[1]):
                ...
                transfer_index = torch.zeros_like(
                    cands0, dtype=torch.bool, device=cands0.device
                )
                ...
                cands_text = tokenizer.batch_decode(
                    cands_copy[:, prompt.shape[1] :], skip_special_tokens=True
                )  # b of strings

                cands_emb = sentence_model.encode(
                    cands_text, convert_to_tensor=True, normalize_embeddings=True
                )
...
def main():
    ...
    prompts = [
        PROMPT_INSTRUCT.format(hate_text=example["en_toxic_comment"], neutral_text="")[
            :-1
        ]
    ...
    with open(out_file, "w") as f_out:
        for i in tqdm(range(0, input_ids.shape[0], test_batch_size)):
            hates = [
                example["en_toxic_comment"]
                for example in ds_test.select(
                    range(i, min(i + test_batch_size, input_ids.shape[0]))
                )
            ]
            out = generate(
                model,
                input_ids[i : i + test_batch_size],
                attention_mask=attention_mask[i : i + test_batch_size],
            ...
            for o, example in zip(output, ds_rows):
                f_out.write(
                    json.dumps(
                        {
                            "hate": example["en_toxic_comment"],
                            "pred": o,
                            "actual": example["en_neutral_comment"],
                        }
                    )
                    + "\n"
                )
